...
public class PID
{
    ...
    /// <returns></returns>
    public float GetOutput(float currentError, float deltaTime, Vector2 minMax)
    {
        _p = currentError;
        _i += _p * deltaTime;
        ...
        return Mathf.Clamp(_p * Kp + _i * Ki + _d * Kd, minMax.x, minMax.y);
    }
}